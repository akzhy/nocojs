...
describe('rollupNocoPlugin', () => {
  ...
  it('should skip processing for excluded file extensions', async () => {
    ...
    const result = await transformFn.call({}, 'console.log("test")', 'test.txt');
  ...
  it('should skip processing for files in node_modules', async () => {
    ...
    const plugin = rollupNocoPlugin();
    const transformFn = plugin.transform as Function;
    const result = await transformFn.call({}, 'console.log("test")', '/path/to/node_modules/package/index.js');
  ...
  it('should process files matching glob patterns', async () => {
    ...
    const result1 = await transformFn.call({}, 'console.log("test")', '/project/src/components/Button.ts');
    ...
    const result2 = await transformFn.call({}, 'console.log("test")', '/project/dist/bundle.js');
  ...
  it('should handle transformation errors gracefully', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('[@nocojs/rollup-plugin] Error processing test.js:'),
      expect.any(Error)
    );
