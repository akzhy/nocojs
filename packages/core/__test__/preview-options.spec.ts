...
import { base64ToSharpImage, defaultTransformOptions, getInput, isFullyTransparent, isImageSingleColor, verifyPreviewCall } from './utils';
...
describe('Preview options', async () => {
  ...
  test('aspect ratio: width is working as expected', async () => {
    ...
    expect(metadata.height).toBe(Math.floor(10 / widthToHeightRatio));
  });
  ...
  test('aspect ratio: height is working as expected', async () => {
    ...
    expect(metadata.width).toBe(Math.floor(10 * widthToHeightRatio));
  });
